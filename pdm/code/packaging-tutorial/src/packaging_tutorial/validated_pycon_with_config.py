...
class Config(pydantic_settings.BaseSettings):
    ...
    USER_AGENT: str = "python-packaging-tutorial"
...
@lru_cache()
def get_programme(config: Config=Config()) -> dict:
    response = config.make_client().get("https://us.pycon.org/2025/schedule/conference.json")
    response.raise_for_status()
    return response.json()


def get_sessions(config: Config=Config()) -> Generator[Session, None, None]:
    programme = get_programme(config=config)
